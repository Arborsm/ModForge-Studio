...
Put Rust shared test helpers in `apps/desktop/src-tauri/tests/support/`; keep feature-specific test helpers next to the test modules only when they are genuinely domain-specific.
...
- `pnpm desktop:dev` launches the full Tauri desktop app.
- `pnpm build` builds the desktop frontend bundle.
- `pnpm lint` runs ESLint for the TypeScript/React workspace.
- `pnpm --filter @modforge/desktop test` runs the desktop Vitest suite.
