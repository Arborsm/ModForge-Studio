...
if (isLinux && mode === 'dev') {
  ...
  const child = spawn(process.execPath, ['./scripts/dev/run-electron-dev.mjs', ...extraArgs], {
    cwd: desktopRoot,
    stdio: 'inherit',
    env: process.env,
  })
...
const commandArgs = isLinux
  ...
  : [path.join(desktopRoot, 'scripts', 'dev', 'run-tauri-cli.cjs'), mode === 'build' ? 'build' : 'dev', ...extraArgs]
