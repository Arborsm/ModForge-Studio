 ...
 * the host's own module instance (React singleton, jsx-runtime, plugin SDK) as
 * a real ESM module with `export` statements — rolldown internal chunks such as
 * `react-vendor.js` have no exports of their own, so the import map cannot
 * point at them directly. Emitted as fixed-name entries (`vendor/<name>.js`).
 *
 ...
 * - plugin-sdk is real ESM, so its production entry is the static
 *   `vendor/plugin-sdk.ts` file (`export *` works).
 * - In dev, all four URLs are served by pluginVendorDevMiddleware, which
 ...
 */
const pluginVendorFacades = ['react', 'react-dom', 'react-jsx-runtime', 'plugin-sdk'] as const
 ...
 * point to the fixed-name vendor facade entries emitted under `vendor/`.
 *
  ...
  return {
    ...
    configureServer(server) {
      server.middlewares.use((req, res, next) => {
        ...
        const match = /^\/vendor\/(react|react-dom|react-jsx-runtime|plugin-sdk)\.js$/.exec(url)
        if (!match) {
          next()
          return
        }
