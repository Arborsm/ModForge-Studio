        ...
        <div key={`${source.modId}:${source.key}:${source.patchIds.join(',')}`} className="panel-list-card px-3 py-3">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <p className="truncate text-sm font-semibold text-[var(--text-primary)]">{source.modName}</p>
              <p className="truncate text-xs text-[var(--text-secondary)]">{source.modPath}</p>
            </div>
