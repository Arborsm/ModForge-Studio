...
describe('buildCompatRegistrations', () => {
  ...
  test('builds multiple registrations for multiple pages', () => {
    const registrations = buildCompatRegistrations([
      plugin({
        pages: [
          { id: 'a', section: 'tools', order: 1, icon: 'images', titleKey: 'a', presentation: 'standalone', projectAccess: 'none' },
          { id: 'b', section: 'tools', order: 2, icon: 'images', titleKey: 'b', presentation: 'standalone', projectAccess: 'none' },
        ],
      }),
    ])
