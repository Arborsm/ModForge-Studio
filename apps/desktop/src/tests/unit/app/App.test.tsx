...
vi.mock('@platform/host', () => ({
  ...
  translateMachineTranslationBatch: vi.fn(),
  translateLocalizationBatch: vi.fn(),
  listenToOfficialLocalizationIndexProgress: vi.fn(async () => () => undefined),
  reviewLocalizationBatch: vi.fn(),
  listLocalizationReviewRuns: vi.fn(async () => ({ records: [], total: 0 })),
  loadLocalizationReviewRun: vi.fn(),
  updateLocalizationReviewIssues: vi.fn(),
  queryAiUsageSummary: vi.fn(async () => ({
    totals: {
      inputTokens: 0,
      outputTokens: 0,
      cachedTokens: 0,
  ...
  queryAiUsageRecords: vi.fn(async () => ({ records: [], total: 0 })),
  exportAiUsage: vi.fn(async () => 0),
  clearAiUsage: vi.fn(async () => ({ removedEvents: 0, removedDailyRows: 0 })),
  inspectOfficialLocalizationIndex: vi.fn(),
  chooseGameDirectory: vi.fn(async () => null),
  rebuildOfficialLocalizationIndex: vi.fn(),
  searchOfficialLocalization: vi.fn(),
  cancelLocalizationJob: vi.fn(async () => undefined),
  resolveLocalizationScope: vi.fn(),
  rebindLocalizationScope: vi.fn(),
  listLocalizationScopes: vi.fn(async () => ({ records: [], total: 0 })),
  loadLocalizationScope: vi.fn(),
  saveLocalizationScopeSettings: vi.fn(),
  listLocalizationGlossary: vi.fn(async () => ({ records: [], total: 0 })),
  upsertLocalizationGlossary: vi.fn(),
  deleteLocalizationGlossary: vi.fn(),
  loadLocalizationStyle: vi.fn(async () => null),
  saveLocalizationStyle: vi.fn(),
  searchTranslationMemory: vi.fn(async () => ({ records: [], total: 0 })),
  recordConfirmedTranslations: vi.fn(),
  deleteTranslationMemory: vi.fn(),
  copyTranslationMemory: vi.fn(),
  importLocalizationKnowledge: vi.fn(),
  exportLocalizationKnowledge: vi.fn(),
}))
...
vi.mock('@app/providers/CpMakerPlatformProvider', () => ({
  CpMakerPlatformProvider: ({ children }: { children: ReactNode }) => <>{children}</>,
}))
