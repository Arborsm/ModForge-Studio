...
function renderGeneratedHostCommands(commandNames: string[]) {
  ...
  return `// Generated by apps/desktop/scripts/gen/generate-host-commands.mjs. Do not edit by hand.\n\n/** Host command protocol names generated from Tauri command function names. */\nexport const HOST_COMMANDS = {\n${entries}\n} as const\n\nexport type HostCommandName = (typeof HOST_COMMANDS)[keyof typeof HOST_COMMANDS]\n`
}

/** Mirrors the renderer in generate-host-commands.mjs; byte-for-byte drift is caught below. */
function renderGeneratedRustInvokeHandler(commands: { module: string; name: string }[]) {
  ...
  const indent = '            '
  const lines = [`${indent}// Generated by apps/desktop/scripts/gen/generate-host-commands.mjs. Do not edit by hand.`]
  for (const module of [...byModule.keys()].toSorted((left, right) => left.localeCompare(right))) {
    lines.push(`${indent}// ${module}`)
    for (const name of (byModule.get(module) ?? []).toSorted((left, right) => left.localeCompare(right))) {
...
async function scanRustCommands() {
  const srcRoot = sourcePath('src-tauri/src')
  ...
  for (const filePath of commandFiles) {
    ...
    for (const match of source.matchAll(/#\[host_command\([^\]]*\)\]\s*pub\s+(?:async\s+)?fn ([a-z][a-z0-9_]*)/g)) {
      commands.push({ module, name: match[1] ?? '', kind: 'macro' })
    }
  }
  return commands
}

/** snake_case → PascalCase; must match the proc-macro's `to_pascal_case`. */
...
describe('frontend module architecture', () => {
  ...
  it('keeps generated host command constants in sync with Tauri command function names', async () => {
    const commandNames = (await scanRustCommands()).map(({ name }) => name)
  ...
  it('keeps the generated Rust invoke handler list in sync with Tauri command function names', async () => {
    const commands = await scanRustCommands()
    const libRs = await readFile(sourcePath('src-tauri/src/lib.rs'), 'utf8')
    const block = libRs
      .match(/\.invoke_handler\(tauri::generate_handler!\[([\s\S]*?)\]\s*\)/)?.[1]
      ?.replace(/^\n/, '')
      .replace(/\n[ \t]+$/, '')
    expect(block).toBe(renderGeneratedRustInvokeHandler(commands))
  })
  ...
  it('keeps typed host command bindings in canonical form', async () => {
    ...
    for (const match of sidecar.matchAll(
      ...
      typedArms.set(match[1] ?? '', { module: match[2] ?? '', paramsType: match[3] ?? '' })
    }

    const violations: string[] = []
    const wrapperNames = new Set(commands.map(({ name }) => name))
    for (const [name] of typedArms) {
      if (!wrapperNames.has(name)) {
        violations.push(`sidecar typed arm ${name} has no matching Tauri wrapper`)
      }
    }

    for (const { module, name, kind } of commands) {
      const arm = typedArms.get(name)
      if (!arm) {
        continue // legacy arm, validated by the wire-name sync test
      }
      if (arm.module !== module) {
        violations.push(`${name}: sidecar arm module ${arm.module} must equal wrapper module ${module}`)
      }
      if (kind === 'macro') {
        // Macro commands have no source-visible struct/impl; the proc-macro
        // derives them. The sidecar arm must still point at the derived type.
        if (arm.paramsType !== `${toPascalCase(name)}Params`) {
          violations.push(`${name}: macro sidecar arm must use ${toPascalCase(name)}Params, found ${arm.paramsType}`)
        }
        continue
      }
      const source = await readFile(sourcePath(`src-tauri/src/${module.replaceAll('::', '/')}.rs`), 'utf8')
      const fnIndex = source.indexOf(`pub async fn ${name}(`)
      const wrapperWindow = fnIndex >= 0 ? source.slice(fnIndex, fnIndex + 1500) : ''
      const executeCall = new RegExp(`runtime::execute\\(\\s*app\\s*,\\s*([A-Z][A-Za-z0-9_]*)\\s*\\{`)
      const call = executeCall.exec(wrapperWindow)
      if (call?.[1] !== arm.paramsType) {
        violations.push(`${name}: wrapper must execute ${arm.paramsType}, found ${call?.[1] ?? 'no typed execute call'}`)
      }
      if (!source.includes(`impl HostCommand for ${arm.paramsType}`)) {
        violations.push(`${name}: missing impl HostCommand for ${arm.paramsType}`)
      }
      if (!source.includes(`const NAME: &'static str = "${name}"`)) {
        violations.push(`${name}: binding NAME must equal the wrapper function name`)
      }
    }
  ...
  it('keeps the generated sidecar dispatch block in sync with typed bindings', async () => {
    ...
    const end = sidecar.indexOf(endMarker)
    expect(start).toBeGreaterThanOrEqual(0)
    expect(end).toBeGreaterThan(start)
    const actual = sidecar.slice(start + startMarker.length, end)

    // rustfmt may rewrap the generated arms, so compare semantically: the
    // whitespace-tolerant typed-arm map plus the sorted arm order.
    const typedArms = new Map<string, { module: string; paramsType: string }>()
    ...
    const expectedArms = new Map<string, { module: string; paramsType: string }>()
    for (const { module, name, kind } of commands) {
      if (kind === 'macro') {
        expectedArms.set(name, { module, paramsType: `${toPascalCase(name)}Params` })
        continue
      }
      const source = await readFile(sourcePath(`src-tauri/src/${module.replaceAll('::', '/')}.rs`), 'utf8')
      const fnIndex = source.indexOf(`pub async fn ${name}(`)
      const wrapperWindow = fnIndex >= 0 ? source.slice(fnIndex, fnIndex + 1500) : ''
      const call = /runtime::execute\(\s*app\s*,\s*([A-Z][A-Za-z0-9_]*)\s*\{/.exec(wrapperWindow)
      if (call?.[1]) {
        expectedArms.set(name, { module, paramsType: call[1] })
      }
    }
    expect(typedArms).toEqual(expectedArms)
    const armOrder = [...actual.matchAll(/crate::host_command_wire!\(([a-z][a-z0-9_]*)\)/g)].map((match) => match[1] ?? '')
    const expectedOrder = [...commands]
      .sort((left, right) => left.module.localeCompare(right.module) || left.name.localeCompare(right.name))
      .map(({ name }) => name)
    expect(armOrder).toEqual(expectedOrder)
  }, 30000)
