...
import { formatGameStateQueryForHub, type GameStateQueryKey } from './EventGameStateQuerySemantics'
...
export function EventGameStateQueryBuilderModal({
  ...
  function draftForDefinition(definition: GameStateQueryDefinition) {
    return draftsByKey[definition.key] ?? createDefaultGameStateQueryClause(definition.key, `draft-${definition.key}`)
  }

  function updateDraftValue(definition: GameStateQueryDefinition, fieldId: string, value: string) {
    setDraftsByKey((current) => {
      const existing = current[definition.key] ?? createDefaultGameStateQueryClause(definition.key, `draft-${definition.key}`)
      return {
        ...current,
        [definition.key]: {
          ...existing,
