...
export default function PlayerAppearanceWindow({
    ...
    return [
      {
        id: 'hat:none',
        label: copy.none,
        active: activeProfile.hatSpriteIndex == null,
        profile: { ...activeProfile, hatSpriteIndex: null },
      },
      ...Array.from({ length: counts.hat }, (_, index) => ({
        id: `hat:${index}`,
        label: `#${index}`,
  ...
  const pagedItems = sectionItems.slice(currentPage * OPTION_PAGE_SIZE, (currentPage + 1) * OPTION_PAGE_SIZE)

  async function handleOpenSaveBrowser() {
    const nextOpen = !saveBrowserOpen
    setSaveBrowserOpen(nextOpen)
    if (!nextOpen || saveLoading || savePreviews.length > 0) {
      return
    }
      ...
      const previews = (
        await Promise.all(
          slots.map(async (summary) => {
            ...
            return {
              summary,
              profile: parsePlayerAppearanceProfileFromSave(file.content, {
