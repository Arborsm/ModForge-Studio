...
export function EventPatchEditor({ patch, draft, onPatchChange, onAddVirtualAsset }: EventPatchEditorProps) {
  ...
  // Parse selected event into structured commands
  ...
  function updateCommand(index: number, newRaw: string) {
    if (!parsedEvent || !selectedKey) return
    const newSegments = [...parsedEvent.segments]
    newSegments[index + 3] = newRaw // +3 because first 3 are scene setup
    const newScript = newSegments.join('/')
