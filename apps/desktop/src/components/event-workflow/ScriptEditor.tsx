// components/event-workflow/ScriptEditor.tsx
// 右侧剧本编辑器容器
...
// import { parseEventCommand, parseEventCommands } from '../../lib/events/parser'
import { serializeRaw } from '../../lib/events/rawSerializer'
import { ScriptTimeline } from './ScriptTimeline'
import { CommandPalette } from './CommandPalette'
import { COMMAND_TEMPLATES } from '../../lib/events/commandTemplates'
...
export function ScriptEditor({
  ...
  useEffect(() => {
    ...
    const storeScript = useEditorStore.getState().currentScript
    ...
    >
      ...
      <CommandPalette
        ...
        onSelect={(key) => {
          ...
          const state = useEditorStore.getState()
          const insertIndex = state.commandPaletteInsertIndex ?? state.currentScript?.commands.length ?? 0
          state.insertCommandAt(insertIndex, template)
        }}
        locale={locale}
      />
    </div>
