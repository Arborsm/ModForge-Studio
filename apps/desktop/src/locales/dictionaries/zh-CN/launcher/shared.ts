...
const shared: LauncherSharedCopy = {
  ...
  logAnalysis: {
    notificationTitle: (count) => `上次游戏过程中日志出现 ${count} 条错误`,
    notificationFirstError: (source, message) => `${source ?? 'SMAPI'}：${message}`,
    analyzeAction: 'AI 解析',
    sheetTitle: '游戏日志错误',
    errorsSectionTitle: (count) => `上次游戏过程中的错误（${count} 条）`,
    errorsTruncated: (count) => `其余 ${count} 条未显示`,
    emptyErrorMessage: '（无错误信息）',
    setupTitle: '设置 AI 解析',
    setupDescription: '选择服务商并粘贴 API Key。Key 仅保存在本机，只用于解析游戏日志错误。',
    providerLabel: '服务商',
    modelLabel: '模型',
    apiKeyLabel: 'API Key',
    apiKeyHint: '以明文保存在本机。',
    saveConfigAction: '保存并开始解析',
    runningLabel: '正在解析日志…',
    resultTitle: 'AI 解析结果',
    failedTitle: '解析失败',
    retryAction: '重试',
    incompleteConfigMessage: '请选择服务商并填写模型和 API Key。',
  },
  states: {
    loading: '正在加载启动器数据...',
    noImage: '暂无图片',
