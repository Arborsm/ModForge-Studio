...
function CatalogViewToggle({
  ...
  return (
    <div className="border-border-subtle bg-surface-panel-muted flex gap-px rounded-lg border p-px" role="group" aria-label="View mode">
      <button
        type="button"
        aria-pressed={mode === 'list'}
        className={cx(
          'inline-flex items-center gap-1.5 rounded-button px-2.5 py-1 text-meta-px font-semibold transition-colors',
          mode === 'list'
            ? 'bg-surface-panel text-text-primary shadow-[inset_0_0_0_1px_color-mix(in_srgb,var(--border-color)_100%,transparent)]'
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0">
        <p className="text-text-primary text-body-px truncate font-bold">{entry.displayName}</p>
        <p className="text-text-tertiary text-meta-px truncate font-mono">{entry.qualifiedItemId}</p>
      </div>
      <p className="text-text-secondary text-body-px truncate">{typeLabel}</p>
      <p className="text-text-primary text-body-px truncate font-bold">{formatPrice(entry.price ?? entry.salePrice, copy)}</p>
      ...
      <div className="panel-body flex min-h-0 flex-1 flex-col overflow-hidden p-4">
        ...
        {browserSourceMode === 'mod' ? (
          modItemGroups.length ? (
            <div className="min-h-0 flex-1 space-y-4 overflow-auto">
              {modItemGroups.map((group) => (
                <section key={group.modPath} className="border-border-subtle bg-surface-panel rounded-2xl border p-4">
                  <div className="mb-3 flex items-center justify-between gap-3">
                    <div className="min-w-0">
                      <p className="text-text-primary truncate text-sm font-semibold">{group.modName}</p>
                      <p className="text-text-secondary truncate text-xs">{group.items.length} modified items</p>
                    </div>
                      ...
                      return (
                        <button
                          ...
                          className={cx(
                            ...
                            isActive
                              ...
                              : 'border-border-subtle bg-surface-panel-muted hover:bg-surface-hover',
                          )}
                        ...
                        >
                          ...
                          <div className="min-w-0 flex-1">
                            <p className="text-text-primary truncate text-sm font-semibold">{entry.displayName}</p>
                            <p className="text-text-secondary truncate text-xs">{targets[0] ?? entry.qualifiedItemId}</p>
                          </div>
                        </button>
                      )
                    })}
          ...
          <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
            <div className="bg-surface-panel-muted text-text-tertiary text-caption grid grid-cols-[2.75rem_1.5fr_0.9fr_0.7fr_0.9fr] gap-3 rounded-lg px-3 py-2 font-extrabold tracking-widest uppercase">
              <span />
        ...
        {browserSourceMode === 'original' && totalItems > 0 ? (
          <div className="border-border-subtle/65 mt-3 flex flex-wrap items-center justify-between gap-3 border-t pt-3">
            <div className="flex flex-wrap items-center gap-2">
              <span className="bg-surface-panel-muted text-text-primary rounded-full px-3 py-1 text-xs font-semibold">
                {rangeStart}-{rangeEnd}
              </span>
              <span className="text-text-secondary text-xs">
                {text.catalogPageLabel} {currentPage} / {pageCount}
              </span>
              <span className="text-text-tertiary text-meta-px">{text.catalogWheelHint}</span>
            </div>
            <div className="flex items-center gap-2">
              <button
                type="button"
                className="border-border-subtle bg-surface-panel-muted text-text-secondary hover:bg-surface-hover hover:text-text-primary inline-flex h-8 items-center gap-1 rounded-md border px-2.5 text-xs font-semibold transition-colors disabled:cursor-not-allowed disabled:opacity-45"
              ...
              <div className="border-border-subtle bg-surface-panel-muted flex items-center gap-1 rounded-md border p-1">
                {paginationTokens.map((token) => {
                  if (token.type === 'ellipsis') {
                    return (
                      <span
                        key={token.key}
                        className="text-text-tertiary inline-flex min-w-8 items-center justify-center px-1 text-xs font-semibold"
                      >
                        ...
                      </span>
                    )
