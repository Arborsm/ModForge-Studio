import { Check, ChevronDown, ChevronUp, Languages, Plus, Save, Search } from 'lucide-react'
...
const statusFilters: ModI18nStatusFilter[] = ['all', 'translated', 'missing', 'error']

/** Common Stardew Valley i18n locales offered as target languages even before the file exists. */
const MOD_I18N_TARGET_LOCALES = ['en', 'zh', 'fr', 'de', 'hu', 'it', 'ja', 'ko', 'pt', 'ru', 'es', 'tr']
...
function statusFilterBackgroundClass(status: ModI18nStatusFilter) {
...
function isStatusHighlighted(status: ModI18nStatusFilter, statusCounts: ReturnType<typeof getStatusCounts>, totalEntries: number) {
...
function statusFilterClass(status: ModI18nStatusFilter, isActive: boolean, highlighted: boolean) {
  return cx(
...
function getStatusCounts(entries: ModI18nEntry[]) {
  return {
...
function useModI18nWorkspaceState({
  projectDetail,
  i18nFiles,
  sourceLocale,
  targetLocale,
  ...
  'projectDetail' | 'i18nFiles' | 'sourceLocale' | 'targetLocale' | 'query' | 'statusFilter' | 'onI18nFilesChange'
>) {
  ...
  const updateEntry = useCallback(
    ...
    [i18nFiles, onI18nFilesChange, projectDetail?.summary.absolutePath, targetFile, targetLocale],
  )
...
export function ModI18nWorkspace({
  ...
  const localeLabels = useMemo(() => {
    ...
    for (const locale of MOD_I18N_TARGET_LOCALES) {
  ...
  const targetOptions: LocaleOption[] = useMemo(() => {
    ...
    for (const locale of candidateLocales) {
      ...
      if (file) {
        const entries = buildModI18nEntries({ sourceFile, targetFile: file, query: '', status: 'all' })
  ...
  const handleTargetLocaleChange = useCallback(
    (nextLocale: string) => {
      ...
      if (!i18nFiles.some((file) => file.locale === nextLocale)) {
        const nextFile = createI18nFile(projectDetail.summary.absolutePath, nextLocale)
        onI18nFilesChange([...i18nFiles, nextFile])
      }
      onTargetLocaleChange(nextLocale)
    },
    [i18nFiles, onI18nFilesChange, onTargetLocaleChange, projectDetail, targetLocale],
  )
        ...
        <header className="flex flex-wrap items-center justify-between gap-4 border-b border-(--border-color)/60 px-5 py-3">
          <div className="flex min-w-0 items-center gap-4">
            <div className="min-w-0">
              <p className="text-[0.625rem] font-bold tracking-[0.16em] text-(--text-tertiary) uppercase">{projectDetail.summary.name}</p>
