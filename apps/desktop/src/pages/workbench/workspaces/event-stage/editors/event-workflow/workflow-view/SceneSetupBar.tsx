  ...
  return (
    <>
      <button
        ...
        type="button"
        className={cx(
          'inline-flex h-7 w-7 cursor-pointer items-center justify-center rounded-md border border-[var(--border-color)] bg-[var(--bg-app)] text-[var(--text-secondary)] hover:text-[var(--text-primary)]',
          triggerClassName,
        )}
        title={label}
        aria-label={label}
      ...
      {open && rect ? (
        <div
          ref={menuRef}
          className="fixed z-[130] grid min-w-32 gap-1 rounded-md border border-[var(--border-color)] bg-[var(--bg-elevated)] p-1 shadow-[var(--shadow-float)]"
...
function CompactIconButton({
  ...
  return (
    <button
      type="button"
      className={cx(
        'inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-md border text-[var(--text-secondary)] transition-colors',
        active
          ? 'border-[color-mix(in_srgb,var(--accent)_55%,var(--border-color))] bg-[color-mix(in_srgb,var(--accent-soft)_78%,transparent)] text-[var(--accent)]'
          : 'border-[var(--border-color)] bg-[var(--bg-app)] hover:border-[color-mix(in_srgb,var(--accent)_42%,var(--border-color))] hover:text-[var(--text-primary)]',
        danger && 'hover:border-[color-mix(in_srgb,var(--danger)_45%,var(--border-color))] hover:text-[var(--danger)]',
        className,
      )}
      title={label}
      aria-label={label}
...
function CoordinateField({
  ...
  return (
    <span
      className={cx(
        'inline-flex h-7 items-center overflow-hidden rounded-md border border-[var(--border-color)] bg-[var(--bg-app)]',
        disabled && 'opacity-55',
      )}
    >
      {(['x', 'y'] as const).map((axis) => (
        <label
          key={axis}
          className="inline-flex h-full items-center gap-1 px-1.5 text-[10px] font-semibold text-[var(--text-tertiary)] uppercase"
        >
          {labels[axis]}
          <input
            type="number"
            className="h-full w-10 bg-transparent font-mono text-[11px] font-semibold text-[var(--text-primary)] outline-none"
            value={axis === 'x' ? x : y}
            disabled={disabled}
...
function DirectionSegmentedControl({ value, onChange }: { value: number; onChange: (value: number) => void }) {
  return (
    <span className="inline-flex h-7 overflow-hidden rounded-md border border-[var(--border-color)] bg-[var(--bg-app)]">
      {DIRECTION_OPTIONS.map(({ value: optionValue, label, icon: Icon }) => (
        <button
          key={optionValue}
          type="button"
          className={cx(
            'inline-flex h-full w-6 items-center justify-center border-r border-[var(--border-color)] text-[var(--text-tertiary)] last:border-r-0 hover:text-[var(--text-primary)]',
            value === optionValue && 'bg-[color-mix(in_srgb,var(--accent-soft)_82%,transparent)] text-[var(--accent)]',
          )}
          title={label}
          aria-label={label}
...
export function SceneSetupBar({
  ...
  return (
    <div
      className={cx('flex min-h-10 items-center gap-2 border-b border-[var(--border-color)] bg-[var(--bg-panel)] px-2 py-1.5', className)}
    >
      <div className="inline-flex h-8 items-center gap-1.5 rounded-md border border-[var(--border-color)] bg-[var(--bg-app)] px-1.5">
        <Music className="h-3.5 w-3.5 shrink-0 text-[var(--text-tertiary)]" />
        <EventResourcePicker
      ...
      <div className="inline-flex h-8 shrink-0 items-center gap-1.5 rounded-md border border-[var(--border-color)] bg-[var(--bg-panel-muted)] px-1.5">
        <Camera className="h-3.5 w-3.5 shrink-0 text-[var(--text-tertiary)]" />
        <div className="inline-flex h-7 overflow-hidden rounded-md border border-[var(--border-color)] bg-[var(--bg-app)]">
          {(['follow', 'continue', 'target'] as const).map((mode) => (
            <button
              ...
              className={cx(
                ...
                cameraMode === mode
                  ? 'bg-[color-mix(in_srgb,var(--accent-soft)_85%,transparent)] text-[var(--accent)]'
      ...
      <div className="flex min-w-0 flex-1 items-center gap-1.5">
        ...
        <div className="flex min-w-0 items-center gap-1 overflow-x-auto overflow-y-visible pb-0.5">
            ...
            return (
              <div
                ...
                className={cx(
                  ...
                  isPicking
                    ? 'border-[var(--accent)] bg-[color-mix(in_srgb,var(--accent-soft)_62%,transparent)] shadow-sm'
                ...
                <FloatingSceneMenu label={labels.more}>
                  {(close) => (
                    <>
                      <button
                        type="button"
                        className="inline-flex h-7 items-center gap-2 rounded px-2 text-left text-[11px] text-[var(--text-secondary)] hover:bg-[var(--bg-panel-muted)] hover:text-[var(--text-primary)]"
                        onClick={() => {
                          commitActors([
                      ...
                      <button
                        type="button"
                        className="inline-flex h-7 items-center gap-2 rounded px-2 text-left text-[11px] text-[var(--danger)] hover:bg-[color-mix(in_srgb,var(--danger)_10%,transparent)]"
                        onClick={() => {
          ...
          <FloatingSceneMenu
            ...
            triggerClassName="w-auto gap-1 border-dashed px-2 text-[11px] font-medium hover:border-[color-mix(in_srgb,var(--accent)_42%,var(--border-color))] hover:text-[var(--accent)]"
          >
            {(close) => (
              <button
                type="button"
      ...
      {onPickModeToggle && (
        <button
          ...
          className={cx(
            ...
            pickMode
              ? 'border-[color-mix(in_srgb,var(--accent)_48%,var(--border-color))] bg-[color-mix(in_srgb,var(--accent-soft)_78%,transparent)] text-[var(--accent)]'
