...
export function PickModeOverlay({
  ...
  return (
    <div className={cx('pointer-events-none absolute inset-x-0 top-3 z-50 flex justify-center px-3', className)} data-pick-mode-overlay>
      <div
        className="pointer-events-auto flex max-w-[min(560px,calc(100%-1rem))] flex-wrap items-center justify-center gap-1.5 rounded-md border border-[color-mix(in_srgb,var(--accent)_42%,var(--border-color))] bg-[color-mix(in_srgb,var(--bg-elevated)_94%,transparent)] px-2 py-1.5 shadow-[var(--shadow-panel)]"
      ...
      >
        <div className="flex h-6 w-6 items-center justify-center rounded border border-[color-mix(in_srgb,var(--accent)_38%,transparent)] bg-[color-mix(in_srgb,var(--accent-soft)_70%,transparent)]">
          <MousePointerClick className="h-3.5 w-3.5 text-[var(--accent)]" />
        </div>
        <p className="max-w-72 truncate text-xs font-semibold text-[var(--text-primary)]">{label ?? '点击地图选择位置'}</p>
        ...
        <p className="text-[11px] text-[var(--text-secondary)]">Esc</p>
      </div>
    </div>
  )
}
