      ...
      <div className="pointer-events-none absolute inset-x-0 bottom-24 flex justify-center px-4">
        {playbackState.pendingChoice ? (
            ...
            <div className="mt-4 grid gap-2 md:grid-cols-2">
              {playbackState.pendingChoice.choices.map((choice, index) => (
                <button
                  key={`${choice.id}:${index}`}
                  type="button"
                  className="event-stage-choice-button panel-list-card panel-list-card-interactive px-4 py-3 text-left text-sm text-(--text-primary)"
          ...
          <div className="panel-overlay-card pointer-events-none flex w-full max-w-4xl items-end gap-4">
            <div className="relative hidden h-64 w-64 shrink-0 overflow-hidden rounded-2xl border border-(--border-color) bg-(--bg-panel) sm:block">
