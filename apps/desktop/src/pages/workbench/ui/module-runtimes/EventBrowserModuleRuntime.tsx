...
export default function EventBrowserModuleRuntime() {
  ...
  const registerStageSeek = (seekTimelineEntry: (entryId: string) => void) => {
    setStageSeek(() => seekTimelineEntry)
    return () => setStageSeek(null)
  }
  const handleActivateTimelineEntry = (entryId: string) => stageSeek?.(entryId)
  const workspacePanels = buildEventsWorkspacePanels({
    copy: props.copy,
    locale: props.locale,
