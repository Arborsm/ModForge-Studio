...
/** Game-shape `Data/Objects` record, as the codex read model consumes it. */
export type ObjectDataFields = {
  Name?: string | null
...
export function createObjectEntryIndex(content: string) {
  return createObjectEntriesFromRecords(JSON.parse(content) as Record<string, ObjectDataFields>)
}

/**
 * Builds the codex read model from already-parsed `Data/Objects` records.
 *
 * The authoring page previews the entry it is editing, which only exists as an
 * in-memory record, so it needs this without a JSON round trip; the codex keeps
 * reading whole assets through `createObjectEntryIndex`.
 */
export function createObjectEntriesFromRecords(parsed: Record<string, ObjectDataFields>) {
