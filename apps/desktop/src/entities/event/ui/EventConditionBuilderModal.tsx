...
import { CONDITION_CATEGORY_ICONS, CONDITION_WEATHER_ICONS, iconForConditionChip } from './eventConditionBuilderChipIcon'
import type { EventPatchHubEvent } from '../model/patchHub'
import { EventPreconditionParser, formatEventPreconditionForHub } from '../model/preconditionSemantics'
import {
  CATALOG_CONDITIONS,
  CATEGORY_IDS as CONDITION_CATEGORY_IDS,
  COMPACT_CHAIN_THRESHOLD,
  DAY_PRESETS,
  DEFAULT_CATALOG_ARGS,
  ITEM_SUGGESTIONS,
  NPC_OPTIONS,
  SEASON_CODES,
  SKILL_OPTIONS,
  ...
  chipNatural,
  clockLabel,
  gameStateQueryFromChip,
  initialChips,
  initialEventId,
  normalizeRangeValue,
  quoteQuery,
  type CatalogConditionDefinition,
  type ChipDragState,
...
export function EventConditionBuilderModal({ event, allEvents, alias, hubCopy, copy, onApply, onCancel }: EventConditionBuilderModalProps) {
  ...
  const visibleCategories = CONDITION_CATEGORY_IDS.filter((category) => {
        ...
        {open ? (
          <div className="condition-catalog-grid">
            {definitions.map((definition) => {
              ...
              const Icon = iconForConditionChip({
        ...
        <section className="condition-builder-card">
          ...
          <div className="condition-weather-row">
            {(Object.keys(copy.weathers) as WeatherId[]).map((weather) => {
              const WeatherIcon = CONDITION_WEATHER_ICONS[weather]
              return (
                <button
                  key={weather}
                  type="button"
    ...
    <div className="event-condition-builder-backdrop" role="presentation" onClick={onCancel}>
      <div className="event-condition-builder-stack" onClick={(event) => event.stopPropagation()}>
          ...
          <main className="condition-builder-body">
            <nav className="condition-builder-rail" aria-label={copy.title}>
              ...
              {visibleCategories.map((category) => {
                const CategoryIcon = CONDITION_CATEGORY_ICONS[category]
                return (
                  <button
                    key={category}
                    type="button"
        ...
        <aside className={cx('condition-builder-chain-dock', hasWeatherConflict && 'conflict')} aria-label={copy.logicChainTitle}>
          <EventConditionBuilderLogicChain
            chips={chips}
            chipDrag={chipDrag}
            ...
            onChipPointerEnd={handleChipPointerEnd}
          />
        </aside>
