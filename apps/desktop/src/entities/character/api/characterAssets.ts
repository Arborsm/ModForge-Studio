 ...
 * caches the hydrated result. The browser page and the authoring page share the
 * cache, so switching between them never re-reads the game directory.
 *
 * Image sheets go through `loadCharacterImageState`, which owns the localized
 * path fallbacks (`*.xx-XX.xnb`) and the Monsters folder's space/underscore
 * spelling drift.
 */
...
/** Resolved image sheet: the path that answered plus its natural dimensions. */
...
const imageStateCache = new Map<string, Promise<CharacterImageState>>()
...
function resolveGiftTasteForCandidate(
  ...
  for (const { key, result: taste } of GIFT_TASTE_ORDER) {
    ...
    if (matchedItemSource) {
      return { taste, source: matchedItemSource }
    }
  }
  ...
  for (const { key, result: taste } of GIFT_TASTE_ORDER) {
    ...
    if (matchedTagSource) {
      return { taste, source: matchedTagSource }
    }
  }
  ...
  if (!matchedUniversalItem && candidate.category !== 0) {
    for (const { key, result: taste } of GIFT_TASTE_ORDER) {
      ...
      if (matchedCategorySource) {
        return { taste, source: matchedCategorySource }
      }
    }
  }
...
/**
 * Loads and hydrates the vanilla character index for one game root and locale.
 * Cached per (root, locale); a failed load drops its cache entry so the next
 * call retries instead of replaying the error forever.
 */
export async function loadCharacterWorkspaceEntries(rootPath: string, locale: LocaleCode) {
...
/**
 * Reads the raw vanilla `Data/Characters` records, keyed by NPC id.
 *
 * The authoring page seeds an override from the untouched record, so it needs
 * the game's JSON rather than the derived index `loadCharacterWorkspaceEntries`
 * returns.
 */
export async function loadVanillaCharacterRecords(rootPath: string, locale: LocaleCode): Promise<Record<string, unknown>> {
...
/** Reads the vanilla `Data/NPCGiftTastes` rows, keyed by NPC id. */
export async function loadVanillaGiftTasteEntries(rootPath: string, locale: LocaleCode): Promise<Record<string, string>> {
  const asset = await loadTextAsset(rootPath, CHARACTER_GIFT_TASTES_ASSET_PATH, locale)
  const parsed = JSON.parse(asset.content) as Record<string, unknown>
