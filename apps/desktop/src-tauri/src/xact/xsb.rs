...
mod tests {
    use super::{
        find_sound_match_for_cue, parse_clip_event_wave, parse_complex_cue_entry, parse_sound_entry_complex_wave,
        parse_sound_entry_direct_wave, parse_xsb_header, read_sound_entries, SoundEntry,
    };
    use std::collections::HashMap;

    #[test]
    fn parses_direct_simple_sound_wave_reference() {
        let bytes = vec![0x00, 0x03, 0x00, 0xB4, 0x00, 0x00, 0x00, 0x0C, 0x00, 0x14, 0x00, 0x00];
        ...
        let parsed = parse_sound_entry_direct_wave(&bytes, &entry).expect("expected direct simple sound parse");
    ...
    fn parses_complex_sound_play_wave_event_type_4() {
        let bytes = vec![
            0x01, 0x03, 0x00, 0x76, 0x00, 0x00, 0x00, 0x3c, 0x00, 0x01, 0xb4, 0x13, 0x00, 0x00, 0x00, 0xc0,
            0x5d, 0xe8, 0x03, 0x01, 0x04, 0x00, 0x00, 0x20, 0x00, 0x00, 0xff, 0x0c, 0x09, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x1e, 0xfe, 0xa0, 0x01, 0x97, 0xd6, 0x00, 0x00, 0x7a, 0x44, 0x00, 0x00,
            0x7a, 0x44, 0x00, 0x00, 0xf0, 0x41, 0x00, 0x00, 0xf0, 0x41, 0x00, 0x10,
        ];
        let entry = SoundEntry {
            index: 0,
            start: 0,
        ...
        let parsed = parse_sound_entry_complex_wave(&bytes, &entry).expect("expected complex sound parse");
    ...
    fn parses_complex_sound_play_wave_event_type_4_with_rpc() {
        let bytes = vec![
            0x03, 0x03, 0x00, 0xbc, 0xa8, 0xfd, 0x00, 0x43, 0x00, 0x01, 0x07, 0x00, 0x01, 0x66, 0x01, 0x00,
            0x00, 0xb4, 0x1a, 0x00, 0x00, 0x00, 0xc0, 0x5d, 0xe8, 0x03, 0x01, 0x04, 0x00, 0x00, 0x20, 0x00,
            0x00, 0xff, 0x0c, 0x0f, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x24, 0xff, 0x23, 0x02, 0xd0,
            0xff, 0x00, 0x00, 0x7a, 0x44, 0x00, 0x00, 0x7a, 0x44, 0x00, 0x00, 0xf0, 0x41, 0x00, 0x00, 0xf0,
            0x41, 0x00, 0x30,
        ];
        let entry = SoundEntry {
            index: 0,
        ...
        let parsed = parse_sound_entry_complex_wave(&bytes, &entry).expect("expected rpc complex sound parse");
    ...
    fn parses_clip_event_type_1_wave_reference() {
        let bytes = vec![0x01, 0x01, 0x00, 0x00, 0x20, 0x00, 0x00, 0xff, 0x0c, 0x2c, 0x00, 0x00, 0xff, 0x00, 0x00, 0x00];
    ...
    fn resolves_complex_cue_variation_mode_1_to_first_valid_sound() {
        ...
        let resolved = find_sound_match_for_cue(&bytes, &header, &sound_entries, 1, &[512], &HashMap::new())
