...
fn network_flood_does_not_delay_io() {
    ...
    network_started_rx
        .recv_timeout(Duration::from_secs(1))
        .expect("network command should start");
    scheduler.submit(test_command(SidecarLane::Io, "io", NO_RESOURCES, || {
        Ok(Value::Null)
    }));
    let response = scheduler.recv();
...
fn panic_returns_error_and_worker_survives() {
    ...
    scheduler.submit(test_command(SidecarLane::Io, "panic", NO_RESOURCES, || {
        panic!("simulated sidecar panic");
    }));
    scheduler.submit(test_command(SidecarLane::Io, "after", NO_RESOURCES, || {
        Ok(Value::Null)
    }));
    let response = scheduler.recv();
