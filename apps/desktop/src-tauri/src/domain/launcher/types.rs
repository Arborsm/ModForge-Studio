...
// Launcher-facing Nexus wire types are produced by the nexusmods domain, so
// they are defined there and re-exported here for launcher-side consumers
// (R4: nexusmods → launcher must stay empty).
pub use crate::domain::nexusmods::types::{
    LauncherCatalogPageResult, LauncherRemoteModDetail, LauncherUpdateChangelogResult,
