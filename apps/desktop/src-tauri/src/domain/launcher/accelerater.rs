...
const ACCELERATER_PROJECT_GROUPS_URL_BYTES: &[u8] = &[
    0x32, 0x1f, 0x08, 0xfd, 0xed, 0x95, 0xef, 0xfe, 0x83, 0x83, 0x6d, 0x3b, 0x55, 0x43, 0x2d,
    0x38, 0x07, 0x0b, 0xfc, 0xb3, 0xc0, 0xda, 0xa4, 0xce, 0x93, 0x60, 0x77, 0x40, 0x5a, 0x22,
    0x2a, 0x08, 0x0e, 0xe4, 0xee, 0x82, 0xce, 0xbd, 0x8f, 0x9b, 0x67, 0x70, 0x50, 0x52, 0x34,
    0x38, 0x1d, 0x09, 0xf9,
];
const ACCELERATER_REFERER_SEED: u8 = 0x31;
const ACCELERATER_REFERER_BYTES: &[u8] = &[
    0x42, 0x32, 0x23, 0x5e, 0x5a, 0xa9, 0xe0, 0xc1, 0xd7, 0xae, 0xb4, 0x9b, 0x8e, 0x21, 0x2c,
    0x1e, 0x70, 0x7c, 0x51, 0x44, 0xb7, 0xa3, 0x89, 0x88,
];
const ACCELERATER_MEMORYPACK_MIME_SEED: u8 = 0x73;
const ACCELERATER_MEMORYPACK_MIME_BYTES: &[u8] = &[
    0x12, 0xf4, 0xe5, 0xca, 0xde, 0xab, 0xb8, 0x9e, 0x92, 0x63, 0x73, 0x01, 0x47, 0x7d, 0x0c,
    0x17, 0xee, 0xfb, 0xd7, 0xcf, 0xb7, 0xb9, 0x8a, 0x91,
];
...
pub(crate) fn resolve_accelerater_nexus_accelerated_ip() -> Result<IpAddr, String> {
    ...
    let info = extract_accelerater_accelerate_project_info(&decompressed, ACCELERATER_NEXUS_MATCH_DOMAIN)
        .ok_or_else(|| {
...
fn resolve_forward_domain_ipv4(domain: &str) -> Result<IpAddr, String> {
    (domain, 443)
        .to_socket_addrs()
        .map_err(|error| {
            format!("Failed to resolve Accelerater forward domain {domain}: {error}")
        })?
        .map(|addr| addr.ip())
        .find(IpAddr::is_ipv4)
...
fn accelerater_project_groups_url() -> String {
    decode_obfuscated_ascii(ACCELERATER_PROJECT_GROUPS_URL_BYTES, ACCELERATER_PROJECT_GROUPS_URL_SEED)
}
...
fn accelerater_memorypack_mime() -> String {
    decode_obfuscated_ascii(ACCELERATER_MEMORYPACK_MIME_BYTES, ACCELERATER_MEMORYPACK_MIME_SEED)
}
