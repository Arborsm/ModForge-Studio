use crate::domain::generated_project::storage::{
    copy_generated_project_draft_at_dir, delete_generated_project_draft_at_dir,
    draft_file_path_at_dir, list_generated_project_drafts_at_dir, load_generated_project_draft_at_dir,
    save_generated_project_draft_at_dir,
};
use crate::domain::generated_project::types::{
    CopyGeneratedProjectDraftRequest, GeneratedProjectDraftErrorCode, GeneratedProjectDraftOperation,
    GeneratedProjectDraftRecord, GeneratedProjectEventSourceSnapshot,
    GeneratedProjectExportFingerprint, GeneratedProjectMetadata, GeneratedProjectOverlayTarget,
    GeneratedProjectOverlayTargetSource,
};
...
fn sample_draft(draft_storage_key: &str) -> GeneratedProjectDraftRecord {
    GeneratedProjectDraftRecord {
        ...
        project_metadata: GeneratedProjectMetadata {
            ...
            project_description: "A generated-project draft".to_string(),
        ...
        overlay_targets: vec![GeneratedProjectOverlayTarget {
            ...
            required: false,
            source: GeneratedProjectOverlayTargetSource::ScannedMod,
        }],
            ...
            "batches": [
                {
                    "id": "batch-001",
                    "workspace": "mods",
                    "label": "Edit generated project metadata"
                }
            ]
        }),
        ...
        last_export_fingerprint: Some(GeneratedProjectExportFingerprint {
...
#[test]
fn saves_loads_and_lists_generated_project_drafts_round_trip() {
    let root = create_temp_dir("generated-project-round-trip");
    let draft = sample_draft("draft-001");

    let saved = save_generated_project_draft_at_dir(&root, draft).expect("save draft");
    let loaded = load_generated_project_draft_at_dir(&root, "draft-001").expect("load draft");
    let listed = list_generated_project_drafts_at_dir(&root).expect("list drafts");
...
#[test]
fn copies_generated_project_drafts_with_a_new_storage_key() {
    let root = create_temp_dir("generated-project-copy");
    let source = save_generated_project_draft_at_dir(&root, sample_draft("draft-source"))
        .expect("save source draft");

    let copied = copy_generated_project_draft_at_dir(
        &root,
        CopyGeneratedProjectDraftRequest {
    ...
    let reloaded = load_generated_project_draft_at_dir(&root, &copied.draft_storage_key)
...
#[test]
fn deletes_generated_project_drafts_and_reports_missing_records_explicitly() {
    let root = create_temp_dir("generated-project-delete");
    save_generated_project_draft_at_dir(&root, sample_draft("draft-delete")).expect("save draft");

    delete_generated_project_draft_at_dir(&root, "draft-delete").expect("delete draft");

    let error = load_generated_project_draft_at_dir(&root, "draft-delete")
        .expect_err("expected deleted draft to be missing");
    assert_eq!(error.code, GeneratedProjectDraftErrorCode::MissingRecord);
    assert_eq!(error.operation, GeneratedProjectDraftOperation::Load);
...
fn surfaces_corrupted_draft_snapshots_as_structured_errors() {
    let root = create_temp_dir("generated-project-corrupted");
    ...
    let error = load_generated_project_draft_at_dir(&root, "draft-corrupted")
        .expect_err("expected corrupted draft load to fail");

    assert_eq!(error.code, GeneratedProjectDraftErrorCode::CorruptedSnapshot);
    assert_eq!(error.operation, GeneratedProjectDraftOperation::Load);
...
#[test]
fn reports_missing_generated_project_drafts_as_structured_errors() {
    let root = create_temp_dir("generated-project-missing");
    let draft_path = draft_file_path_at_dir(&root, "draft-missing");

    let error = load_generated_project_draft_at_dir(&root, "draft-missing")
