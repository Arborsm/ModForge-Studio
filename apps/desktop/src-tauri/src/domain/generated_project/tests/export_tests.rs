...
use crate::domain::generated_project::{
    export_generated_project_pack, types::GeneratedProjectExportRequest,
};
use crate::domain::generated_project::types::{
    GeneratedProjectDraftErrorCode, GeneratedProjectDraftOperation, GeneratedProjectExportResult,
};
...
#[test]
fn exports_generated_project_pack_to_a_fresh_directory() {
    let root = create_temp_dir("generated-project-export-success");
    ...
    let request = GeneratedProjectExportRequest {
        output_path: output_dir.to_string_lossy().into_owned(),
    ...
    let result = export_generated_project_pack(request).expect("export generated project pack");
...
#[test]
fn rejects_generated_project_export_when_target_directory_is_not_fresh() {
    let root = create_temp_dir("generated-project-export-non-empty");
    ...
    let error = export_generated_project_pack(GeneratedProjectExportRequest {
        output_path: output_dir.to_string_lossy().into_owned(),
        manifest_json: manifest.to_string(),
        content_json: content.to_string(),
    ...
    assert_eq!(error.code, GeneratedProjectDraftErrorCode::InvalidExport);
    assert_eq!(error.operation, GeneratedProjectDraftOperation::Export);
...
#[test]
fn rejects_generated_project_export_when_output_path_contains_parent_component() {
    let root = create_temp_dir("generated-project-export-parent-component");
    let output_dir = root.join("fresh").join("..");

    let error = export_generated_project_pack(export_request(&output_dir, Vec::new()))
...
#[test]
fn rejects_generated_project_export_when_virtual_asset_collides_with_reserved_output() {
    let root = create_temp_dir("generated-project-export-reserved-collision");
    let output_dir = root.join("Collision Export");

    let error = export_generated_project_pack(export_request(
        &output_dir,
...
#[test]
fn rejects_generated_project_export_when_virtual_asset_case_collides_with_reserved_output() {
    let root = create_temp_dir("generated-project-export-reserved-case-collision");
    let output_dir = root.join("Collision Export");
...
#[test]
fn rejects_generated_project_export_when_virtual_assets_normalize_to_same_path() {
    let root = create_temp_dir("generated-project-export-duplicate-assets");
    let output_dir = root.join("Collision Export");
...
#[test]
fn rejects_generated_project_export_when_virtual_assets_only_differ_by_case() {
    let root = create_temp_dir("generated-project-export-case-duplicate-assets");
    let output_dir = root.join("Collision Export");
...
#[test]
fn rejects_generated_project_export_when_virtual_assets_only_differ_by_unicode_case() {
    let root = create_temp_dir("generated-project-export-unicode-case-duplicate-assets");
    let output_dir = root.join("Collision Export");
...
fn exported_json_files_keep_pretty_format_with_trailing_newline() {
    let root = create_temp_dir("generated-project-export-json");
    ...
    export_generated_project_pack(GeneratedProjectExportRequest {
        output_path: output_dir.to_string_lossy().into_owned(),
        manifest_json: manifest.to_string(),
        content_json: content.to_string(),
        virtual_assets: Vec::new(),
    })
    .expect("export generated project pack");
