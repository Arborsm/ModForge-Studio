use crate::domain::generated_project::build_generated_project_map_asset;
use crate::domain::generated_project::types::{
    BuildGeneratedProjectMapAssetRequest, GeneratedProjectDraftErrorCode,
...
#[test]
fn build_generated_project_map_asset_serializes_map_document_as_tbin_preview_asset() {
    let request = BuildGeneratedProjectMapAssetRequest {
        relative_path: "assets/maps/preview-town.tbin".to_string(),
        map_document: sample_map_document(),
    };

    let asset = build_generated_project_map_asset(request).expect("build map preview asset");
...
#[test]
fn build_generated_project_map_asset_rejects_non_relative_asset_paths() {
    let error = build_generated_project_map_asset(BuildGeneratedProjectMapAssetRequest {
        relative_path: "../assets/maps/preview-town.tbin".to_string(),
        map_document: sample_map_document(),
    })
    ...
    assert_eq!(error.operation, GeneratedProjectDraftOperation::BuildMapAsset);
