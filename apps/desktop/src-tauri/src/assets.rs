...
fn split_localized_stem(stem: &str) -> (&str, Option<&str>) {
...
fn localized_variant_path(path: &Path, locale: &str) -> Option<PathBuf> {
...
fn logicalized_asset_path(path: &Path) -> PathBuf {
...
fn preferred_existing_xnb_path(path: &Path, locale: Option<&str>) -> PathBuf {
...
fn encode_texture_png(texture: &crate::xnb::TextureData) -> Result<Vec<u8>, String> {
...
mod tests {
    ...
    fn create_test_directory(name: &str) -> PathBuf {
        let unique = SystemTime::now()
            ...
            .expect("system time")
            .as_nanos();
        let path = std::env::temp_dir().join(format!("modforge-assets-{name}-{unique}"));
    ...
    fn prefers_localized_existing_xnb_variant() {
        let root = create_test_directory("localized-xnb");
