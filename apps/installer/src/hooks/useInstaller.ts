    ...
    ;(async () => {
      try {
        ...
        if (context.mode === 'uninstall') {
          ...
          setStep('uninstall')
          // Frameless window: the OS-level title only shows in taskbar / Alt-Tab.
          void getCurrentWindow().setTitle(i18n.t('uninstall.windowTitle'))
    ...
    if (MOCK_INSTALL_FOR_DEBUG) {
      ...
      await new Promise<void>((resolve) => {
        const timer = window.setInterval(() => {
          ...
          const mockStep = percent < 20 ? 'prepare' : percent < 50 ? 'extract' : percent < 75 ? 'config' : 'complete'
